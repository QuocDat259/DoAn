    ...
    public class HomeController : Controller
    {
        ...
        public ActionResult AddReview(DanhGia review)
        {
            ...
            else
            {
                ...
                TinTuc tintuc = db.TinTuc.Find(review.Id_tintuc);
                tintuc.DanhGia = db.DanhGia.Where(r => r.Id_tintuc == review.Id_tintuc).ToList();
        ...
        public ActionResult AddReviewNhaSi(DanhGiaNhaSi reviewnhasi)
        {
            ...
            if (User.Identity.IsAuthenticated)
            {
                ...
                // Lưu đánh giá và bình luận vào cơ sở dữ liệu
                db.DanhGiaNhaSi.Add(reviewnhasi);
                db.SaveChanges();
            ...
            {
                // Nếu dữ liệu không hợp lệ, quay lại trang Details với thông tin nha sĩ và các đánh giá và bình luận đã nhập trước đó
                AspNetUsers nhasi = db.AspNetUsers.Find(reviewnhasi.Id_Nhasi);
                nhasi.DanhGiaNhaSi = db.DanhGiaNhaSi.Where(r => r.Id_Nhasi == reviewnhasi.Id_Nhasi).ToList();
